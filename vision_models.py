...
class AamModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        model = ram(pretrained="/public26_data/lsx/vipergpt/pretrained_models/ram/ram_swin_large_14m.pth",
                    image_size=384,
                    vit='swin_l')
        model.eval()
    ...
    def forward(self, image: torch.Tensor):
        ...
        image = self.transform(image_source).unsqueeze(0).to(device)
...
class GroundingDinoModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, box_threshold=config.detect_thresholds.GD_BOX, text_threshold=config.detect_thresholds.GD_TEXT):
        ...
        with HiddenPrints("GD"):
            model = load_model("/public26_data/lsx/groundingdino/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py", "/public26_data/lsx/groundingdino/weights/groundingdino_swint_ogc.pth")
            model.eval()
        self.model = model
    ...
    @torch.no_grad()
    def forward(self, image: torch.Tensor, obj: Union[str, list], Box_Threshold, return_labels: bool = False):
        transform = T.Compose(
            [
        ...
        boxes1 = box_convert(boxes=boxes, in_fmt="cxcywh", out_fmt="xyxy").numpy()
        boxes2 = boxes1[:, [0, 3, 2, 1]]
        boxes2[:, 1] = h - boxes2[:, 1]
        boxes2[:, 3] = h - boxes2[:, 3]
...
class Easyocr(BaseModel):
    ...
    def forward(self, image: torch.Tensor):
        ...
        with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as temp_file:
            # 保存图像到临时文件
            image_source.save(temp_file.name, format='JPEG')

            # 获取临时文件的路径
            temp_file_path = temp_file.name
        result = self.reader.readtext(temp_file_path, detail = 0)
        return result
...
class TCLModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        checkpoint_path = "/public26_data/lsx/vipergpt/pretrained_models/TCL/TCL_4M.pth"
...
class BLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, half_precision=config.blip_half_precision,
        ...
        with warnings.catch_warnings(), HiddenPrints("BLIP"), torch.cuda.device(self.dev):
            ...
            self.processor = Blip2Processor.from_pretrained(f"/public26_data/lsx/vipergpt/pretrained_models/{blip_v2_model_type}")
            # Device_map must be sequential for manual GPU selection
            try:
                self.model = Blip2ForConditionalGeneration.from_pretrained(
                    f"/public26_data/lsx/vipergpt/pretrained_models/{blip_v2_model_type}", load_in_8bit=half_precision,
